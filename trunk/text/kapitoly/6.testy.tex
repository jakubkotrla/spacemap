...
Jednak testy finálního modelu prostorové mapy na rùznıch datech, druhak testy modelu s rùznımi hodnotami nìkterıch klíèovıch parametrù.
\\
...
\section{Testy finálního modelu} \label{chapter-test-final}
...
Vısledky v dalších místnostech, další vıstupní data a videa zachycující prùbìh testù jsou na pøiloeném CD.
\\
...
\begin{figure}[p]
  ...
  \caption{Test finálního modelu, místnost CrazyRoom.}
...
\begin{figure}[p]
  ...
  \caption{Test finálního modelu, místnost HeapLineRoom.}
...
\begin{figure}[h!]
  ...
  \caption{Test finálního modelu, místnost Lobby.}
...
Jedním z objektivních kritérií modelu je pøenost mapy, nakolik odpovídá realitì virtuálního svìta.

\begin{definition}[Nauèenost pøedmìtu]
V prùbìhu simulace agent vidí a pouívá pøedmìty a prostorová mapa dostává vjemy, které to odráí.
Nauèenost pøedmìtu je vlastnost vyjadøující, jak moc agent pøedmìt vnímal a pouival.
Na zaèátku simulace je rovna nule. V prùbìhu simulace, pokadé, kdy je vytvoøen vjem pøedmìtu, je nauèenost pøedmìtu zvıšena o intenzitu vjemu.
...
Vizualizace dat pro místnost FullRoom je na obrázku \ref{fig:test-heatmap}, další jsou na pøiloeném CD.
Ukazuje, e náš pøepoklad je správnı, uzly prostorové mapy, reps. jejich naplnìnost se blíí nauèenosti pøedmìtù.
\\
...
Dalším mìøítkem je pøesnost zapamatovanıch poloh pøedmìtù. V kadém kroku simulace mùeme urèit, na jaké poloze se agent odmnívá, e pøedmìt je.
To mùeme porovnat s realitou.
...
èase\footnote{Vısledky v dalších místnostech jsou opìt na pøiloeném CD.}.
...
Model prostorové mapy je díky nìmu schopen rozpoznat oblasti s vyšší hustotou uzlù a vytvoøit v takové oblasti místo.
...
Je vidìt, e zmìna testovanıch parametrù zpùsobila vznik více míst. I v témìø prázné místnosti vznikla tøi místa.
Vedlejším efektem však bylo vytvoøení míst i tam, kde jsme je neoèekávali a nechtìli.
Zvıraznìná místa na obrázku \ref{fig:test-places-lh-heap} nevznikly pøímo díky hustotì uzlù.
Ke svému vzniku spotøebovali mnoství odpudivosti nahromadìné v uzlech v tìsné blízkosti jinıch míst.
\\
...
Provedli jsme tedy test v místnosti SwitchRoom, kterou jsme navrhli právì pro tento úèel.
...
Prùbìh testu je lépe vidìt na videu, které je na pøiloeném CD.
\\
